...
const int groupNumber = 10; // Group 10 -> IP will be 192.168.0.11 to avoid router conflicts!
...
<body>
  ...
  <script>
    ...
    function processKeys() {
      ...
      ['btn-fwd', 'btn-bwd', 'btn-lft', 'btn-rgt', 'btn-fl', 'btn-fr', 'btn-bl', 'btn-br'].forEach(function(id) { highlightBtn(id, id === activeBtn); });
    ...
    document.addEventListener('keydown', function(e) { var key = e.key.toLowerCase(); if (keyState[key] !== undefined) { if (!keyState[key]) { keyState[key] = 1; processKeys(); } e.preventDefault(); } });
...
void setup() {
  ...
  // Always configure static IP (192.168.0.11 since groupNumber is 10)
